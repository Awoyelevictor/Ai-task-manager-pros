...
const App: React.FC = () => {
  ...
  return (
    <div className={`flex flex-col min-h-screen max-w-2xl mx-auto pb-48 relative transition-colors duration-1000 ${theme.bg} ${theme.text}`}>
      {/* Alarm Overlay */}
      {activeAlarm && (
      ...
      {/* Header & Greeting */}
      <header className="p-6 pb-2 space-y-6">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            <Sparkles className={`w-6 h-6 ${theme.accent}`} />
            <h1 className="text-2xl font-black tracking-tight">Master Pro</h1>
          </div>
        ...
        <div className={`bg-gradient-to-br ${theme.gradient} rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden group`}>
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div className="space-y-1">
              <h2 className="text-2xl font-black">{greeting?.text}</h2>
              <p className="text-white/80 font-medium">{greeting?.subtext}</p>
            </div>
            <div className="bg-white/20 p-4 rounded-3xl backdrop-blur-md">
              {greeting?.icon === 'sun' && <Sun className="w-8 h-8" />}
              {greeting?.icon === 'sunset' && <Sunset className="w-8 h-8" />}
              {greeting?.icon === 'moon' && <Moon className="w-8 h-8" />}
              {greeting?.icon === 'party' && <PartyPopper className="w-8 h-8" />}
              {greeting?.icon === 'holiday' && (currentTheme === 'christmas' ? <TreePine className="w-8 h-8" /> : <Ghost className="w-8 h-8" />)}
            </div>
          </div>
        </div>

        {/* Input Card */}
        <form onSubmit={addTask} className="bg-white p-5 rounded-3xl shadow-xl border border-slate-100 space-y-4">
          <div className="flex gap-3">
            <div className="flex-grow relative group">
              <LayoutList className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
              <input type="text" value={taskInput} onChange={(e) => setTaskInput(e.target.value)} placeholder="Next objective..." className="w-full pl-12 pr-12 py-4 bg-slate-50/50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-400 transition-all text-slate-800 font-semibold" />
          ...
          <div className="flex items-center gap-3">
            <div className="relative group min-w-[120px]">
              ...
              <select value={taskPriority} onChange={(e) => setTaskPriority(e.target.value as any)} className="appearance-none w-full pl-4 pr-10 py-3 bg-slate-50/50 border-none rounded-xl text-xs font-bold text-slate-600 focus:ring-2 focus:ring-indigo-400 cursor-pointer">
                <option value="low">Low Priority</option>
                <option value="medium">Medium</option>
                <option value="high">High Priority</option>
              </select>
            </div>
            <input type="datetime-local" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="flex-grow pl-4 pr-4 py-3 bg-slate-50/50 border-none rounded-xl text-xs font-bold text-slate-600" />
            <button type="submit" className={`${theme.primary} text-white px-8 py-3 rounded-xl font-bold shadow-lg active:scale-95 transition-transform`}>Add</button>
          </div>
        </form>
      </header>
      ...
      <main className="p-6">
        <div className="space-y-4 mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            <div className="flex bg-white/50 p-1 rounded-xl shadow-sm border border-slate-100">
              {(['all', 'pending', 'completed'] as const).map((status) => (
                <button key={status} onClick={() => setFilterStatus(status)} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all capitalize ${filterStatus === status ? `${theme.primary} text-white shadow-md` : 'text-slate-500 hover:text-indigo-600'}`}>{status}</button>
              ))}
            </div>
            <div className="flex bg-white/50 p-1 rounded-xl shadow-sm border border-slate-100">
              {(['all', 'high', 'medium', 'low'] as const).map((priority) => (
                <button key={priority} onClick={() => setFilterPriority(priority)} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all capitalize ${filterPriority === priority ? `${theme.primary} text-white shadow-md` : 'text-slate-500 hover:text-indigo-600'}`}>{priority === 'all' ? 'All' : priority}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">{filterStatus === 'all' ? 'Your Missions' : `${filterStatus.charAt(0).toUpperCase() + filterStatus.slice(1)} Missions`}<span className="ml-2 text-sm opacity-50 font-medium">({sortedTasks.length})</span></h2>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 flex flex-col gap-4 z-40 pointer-events-none">
        <div className="flex justify-center items-end gap-3 pointer-events-auto max-w-2xl mx-auto w-full px-4">
          <button 
            onClick={handleAskAI} 
            className={`flex-grow group relative flex items-center justify-center gap-3 py-5 bg-slate-900 text-white rounded-2xl font-black text-lg shadow-2xl active:scale-95 transition-all`}
          >
          ...
          <button 
            ...
            className={`flex-shrink-0 p-5 rounded-2xl shadow-2xl bg-gradient-to-br ${theme.gradient} text-white active:scale-90 transition-all border-2 border-white/20`}
            title="AI Assistant"
          >
        ...
        <div className="w-full max-w-2xl mx-auto pointer-events-auto px-4">
          <AdBanner />
        </div>
      </div>
