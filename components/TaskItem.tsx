...
const TaskItem: React.FC<TaskItemProps> = ({ task, onToggle, onUpdate, onDelete }) => {
  ...
  return (
    <div 
      className={`group relative flex items-center gap-4 p-5 rounded-2xl transition-all duration-300 border ${
        task.completed 
        ...
        {isEditing ? (
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative group">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1.5 bg-slate-50 border-none rounded-lg text-[10px] font-black text-slate-600 outline-none"
                />
              </div>
              ...
              <div className="relative group">
                ...
                <select
                  ...
                  className="appearance-none pl-3 pr-7 py-1.5 bg-slate-50 border-none rounded-lg text-[10px] font-black text-slate-600 outline-none capitalize"
                >
              ...
              <div className="flex-grow flex justify-end gap-1">
                ...
                <button onClick={handleCancel} className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              {!task.completed && (
                <span className="text-[8px] text-slate-300 opacity-0 group-hover/text:opacity-100 transition-opacity font-bold uppercase tracking-widest italic">
                  Tap to edit
                </span>
              )}
            </div>
            
            <h3 className={`text-base font-bold transition-all truncate ${
